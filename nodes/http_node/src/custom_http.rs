...
pub fn send_http_get_request(params: InputData) -> Result<Response> {
    fn header_val(v: &str) -> Vec<u8> {
        v.to_string().into_bytes()
    }

    let additional_headers = match params.headers {
        Some(val) => {
            let a = val
                .iter()
                .map(|x| (x.key.clone(), x.value.as_bytes().to_vec()))
                .collect();
            a
        }
        None => vec![],
    ...
    let headers = http_types::Headers::from_list(
        &[
            ...
            additional_headers.as_slice(),
        ]
        .concat(),
    )?;

    let parsed_url = Url::parse(&params.url).map_err(|e| anyhow!("Failed to parse URL: {e}"))?;
    ...
    let authority = parsed_url
        .host_str()
        .ok_or_else(|| anyhow!("URL does not have a valid host"))?;

    let path_with_query = parsed_url.path().to_string()
        + parsed_url
            .query()
            .map(|q| format!("?{}", q))
            .as_deref()
    ...
    options
        .set_connect_timeout(params.connect_timeout)
        .map_err(|()| anyhow!("failed to set connect_timeout"))?;
    options
        .set_first_byte_timeout(params.first_by_timeout)
        .map_err(|()| anyhow!("failed to set first_byte_timeout"))?;
    options
        .set_between_bytes_timeout(params.between_bytes_timeout)
